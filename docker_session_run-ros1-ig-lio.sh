...
IMAGE_NAME='ig-lio_noetic'
TMUX_SESSION='ros1_session'

DATASET_CONTAINER_PATH='/ros_ws/dataset/recorded-ig-lio.bag'
...
docker run -it --rm \
  ...
  -v "$RESULT_IG_LIO_HOST_PATH":"$RESULT_IG_LIO_CONTAINER_PATH" \
  "$IMAGE_NAME" \
  /bin/bash -c '
...
source /ros_ws/devel/setup.bash
echo "[record] start"
rosbag record /current_scan /lio_odom -O '"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME"'
echo "[record] exit"
'\'' C-m
...
docker run -it --rm \
  ...
  /bin/bash -c "
    ...
    source /opt/ros/noetic/setup.bash
    source /ros_ws/devel/setup.bash
    rosrun ig-lio-to-hdmapping listener  \
      \"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME\" \
      \"$BAG_OUTPUT_CONTAINER/$HDMAPPING_OUT_NAME-ig-lio\"
  "
